...
int64_t botje_base64__decode(unsigned char * str_dst, unsigned char *str_src, int64_t str_src_len) {
  ...
  while(remainder > 0) {
    ...
    if(cur_src[2] == '=' && cur_src[3] == '=') {
      int64_t combined = botje_base64__decode_ascii_to_base64(cur_src[0]) << 18;
      combined        += botje_base64__decode_ascii_to_base64(cur_src[1]) << 12;
      ...
      *(cur_dst++) = (combined >> 16) & 0b11111111;
      ...
      int64_t combined = botje_base64__decode_ascii_to_base64(cur_src[0]) << 18;
      combined        += botje_base64__decode_ascii_to_base64(cur_src[1]) << 12;
      combined        += botje_base64__decode_ascii_to_base64(cur_src[2]) << 6;
      ...
      *(cur_dst++) = (combined >> 8) & 0b11111111;
      ...
      combined        += botje_base64__decode_ascii_to_base64(cur_src[1]) << 12;
      combined        += botje_base64__decode_ascii_to_base64(cur_src[2]) << 6;
      combined        += botje_base64__decode_ascii_to_base64(cur_src[3]);
...
int64_t botje_base64__decode_ascii_to_base64(int64_t chr) {
  ...
  printf("botje_base64__decode_ascii_to_base64 error input: |%lld|\n", chr);
  abort();
